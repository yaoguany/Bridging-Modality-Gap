...
"""
Prepare the PGPS9K dataset for contrastive training.
"""
...
DEFAULT_OUTPUT_SUBDIR = "geo9k-contrastive-text-condition"
DEFAULT_INSTRUCTION = (
...
def construct_full_condition(data_dict: dict) -> str:
...
def prepare_split(split_name: str, split_data: Dict[str, dict], diagram_dir: Path) -> List[dict]:
    ...
    for sample in split_data.values():
        prompts_with.append(construct_full_condition(sample))
        prompts_without.append(f"Based on these conditions in the image, answer the question: {sample['text']}")
    ...
    raw_examples = []
    for prompt_with, prompt_without, answer, image in zip(
        prompts_with, prompts_without, answers, encoded_images
    ):
        raw_examples.append(
            {
                "prompt": prompt_with,
                "contrastive_prompt": prompt_without,
                "solution": answer,
                "answer": answer,
                "input_image": image,
                "is_contrastive": True,
...
def make_map_fn(split: str):
    def process_fn(example: dict, idx: int) -> dict:
        ...
        contrastive_raw = example.pop("contrastive_prompt")

        question_with_condition = prompt_raw
        question_without_condition = contrastive_raw

        prompt = "<image>" + question_with_condition + "\n" + DEFAULT_INSTRUCTION
        contrastive_prompt = "<image>" + question_without_condition + "\n" + DEFAULT_INSTRUCTION
        ...
        return {
            ...
            "extra_info": {
                ...
                "index": idx,
                "answer": answer_raw,
                "question_with_condition": question_with_condition,
                "question_without_condition": question_without_condition,
            },
        }
...
def main():
    ...
    train_examples = prepare_split("train", train_data, DIAGRAM_DIR)
    test_examples = prepare_split("test", test_data, DIAGRAM_DIR)
